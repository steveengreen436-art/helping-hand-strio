...
export default function AuthPage() {
  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    if (isLogin) {
      ...
      // Sign up and pass full_name/phone in metadata
      // The database trigger will handle creating the user_profiles row
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen text-white">
      ...
      <form onSubmit={handleAuth} className="flex flex-col gap-4 w-80">
        {!isLogin && (
          <>
            ...
            <input type="tel" placeholder="Phone Number" required className="p-3 bg-white text-black" onChange={(e) => setPhone(e.target.value)} />
          </>
        )}
      ...
      <div className="mt-4 flex flex-col items-center gap-2">
        ...
        {isLogin && (
          <button type="button" onClick={handleResetPassword} className="text-xs text-neonCyan underline">
            Forgot Password?
          </button>
        )}
      </div>
